...
#initiate
g_t_real = cos(2*pi*freq*time) 
g_t_im = sin(2*pi*freq*time)*i 
gt_df_real <- data.frame(time = time, g_t = g_t_real)
gt_df_im <- data.frame(time = time, g_t = g_t_im)
g_hat_mean_real = matrix(0, ncol = 2, nrow = length(samp_f))
g_hat_mean_im = matrix(0, ncol = 2, nrow = length(samp_f))

# Mean of Re and Im for plot of the center of mass further below
for(index in 1:length(samp_f)){
  g_hat_mean_real[index,1] = Re(mean(g_t_real*exp(-2*pi*i*samp_f[index]*time)))
  g_hat_mean_real[index,2] = Im(mean(g_t_real*exp(-2*pi*i*samp_f[index]*time)))
  g_hat_mean_im[index,1] = Re(mean(g_t_im*exp(-2*pi*i*samp_f[index]*time)))
  g_hat_mean_im[index,2] = Im(mean(g_t_im*exp(-2*pi*i*samp_f[index]*time)))
  
  } # End for index

# Turn into data.frame:
g_hat_mean_df_real = as.data.frame(g_hat_mean_real)
colnames(g_hat_mean_df_real) = c("Re","Im")
g_hat_mean_df_im = as.data.frame(g_hat_mean_im)
colnames(g_hat_mean_df_im) = c("Re","Im")
...
for(t in 1:(length(time)-1)){ 
  ...
  plot1 =  ggplot(data = g_f, aes(x = Re, y = Im)) +
    ...
    theme(plot.title.position = "plot",  plot.title = element_text(hjust = 0.7)) +
    coord_fixed()
  ...
  plot2 =  ggplot(data = g_f, aes(x = Re, y = Im)) +
    ...
    geom_segment(data = segment, 
                 ...
                 arrow = arrow(length = unit(0.2, "cm")),
                 color = "black") +
    labs(title = paste( samp_f[index], "Hz"), 
         x = "Real Number", y = "Imaginary Number") +
    xlim(c(min(min_max[,1]),max(min_max[,3]))) +  # making sure that plot is not out of bounds
    ylim(c(min(min_max[,2]),max(min_max[,4]))) +  # and not just centralized
    theme_minimal() + # white instead of grey background
...
create_gif_from_plots(plots, "fft_animation.gif")
